...
if (sessionStorage.getItem('formSubmitted') === 'true') {
    window.location.href = 'tmp-master/index.html'; // Redirect jika form sudah disubmit
}
...
// }
function submitForm() {
    const scriptURL = 'https://script.google.com/macros/s/AKfycbwT2Btz0QMvapt-2v9SYAoAcMyJ5gttwX19RL5tna8ifk7v5Egut3hwRmp-gbB2gsRz/exec';
    const form = document.forms['submit-to-google-sheet'];
    const formData = new FormData(form); // Ambil data dari form
    ...
    .then(response => {
        if (response.ok) {
            sessionStorage.setItem('formSubmitted', 'true'); // Tandai bahwa form sudah disubmit
            window.location.href = 'tmp-master/index.html'; // Redirect ke halaman lain setelah sukses
        } else {
